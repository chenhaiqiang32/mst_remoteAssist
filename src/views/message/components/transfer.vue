<template>
  <div class="transfer-modal-container">
    <div class="transfer-fixed" @click="handleClose">
      <img :src="closeIcon" alt="" />
    </div>
    <div class="transfer-m-t">
      {{ $t('message.chat.group.transfer.title') }}
    </div>
    <div class="transfer-m-s">
      <a-input
    ...
    <div class="transfer-m-m wsl-scroll">
      <div
        v-for="(item, index) in memberList"
        :key="`invite_psn_${index}_${item.userId}`"
        class="transfer-m-m-i"
        @click="handleSelectRow(item)"
      >
        <div class="transfer-m-m-i-l">
          <img
            :src="isUserSelected(item.userId) ? selectActiveIcon : selectIcon"
            alt="avatar"
          />
        </div>
        <div class="transfer-m-m-i-r">
          <div class="transfer-m-m-i-rl">
            <img :src="item.avatarUrl" alt="" />
            <div class="transfer-m-m-i-rl-s" :class="showPsnOnlineType(item)">
            </div>
          </div>
          <div class="transfer-m-m-i-rr">
            <div class="transfer-m-m-i-rr-t">
              <span>{{ item.name }}</span>
            </div>
            <div class="transfer-m-m-i-rr-b">
    ...
    <div class="transfer-m-b">
      <div class="transfer-m-b-l">
        <template
      ...
      <div class="transfer-m-b-r">
        <div class="transfer-m-b-r-btn" @click="handleSureTransfer">
          <span>{{ t('message.chat.group.transfer.btn') }}</span>
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['handleClose', 'handleSureTransfer']);
  const { t } = useI18n();
  const handleClose = () => {
    emit('handleClose');
  };
  ...
  const handleSureTransfer = () => {
    emit('handleSureTransfer', selectPsnList.value);
  };
