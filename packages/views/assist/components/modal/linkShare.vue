  ...
  <MstModal>
    <div class="link-modal-container">
      ...
      <div class="link-m-t borderB">
        {{ t('mst.invite.link.modal.title') }}
      </div>
      <div class="link-m-m">
        <div class="link-m-m-m">
          <div class="link-m-m-mt">
            <span class="name">{{ linkInfo?.name }}</span>
            <span>{{ t('mst.invite.link.modal.text1') }}</span>
          </div>
          <div class="link-m-m-md">
            <span>{{ t('mst.invite.link.modal.text2') }}:</span>
            <span>{{ linkInfo?.subject }}</span>
          </div>
          <div class="link-m-m-mm">
            <span>{{ t('mst.invite.link.modal.text5') }}:</span>
            ...
            <span v-if="linkInfo?.planType === 2">{{
              t('mst.invite.link.modal.text6')
            }}</span>
          </div>
          <div class="link-m-m-mf">
            <span>{{ t('mst.invite.link.modal.text4') }}:</span>
            <span>{{ linkInfo?.meetingNo }}</span>
          </div>
          <div class="link-m-m-mb">
            <span>{{ t('mst.invite.link.modal.text3') }}:</span>
      ...
      <div class="link-m-b">
        <div class="link-m-b-btn">
          <div class="link-m-b-btn-i btn-border" @click="handleCopyAll">
            <span>{{ t('mst.invite.link.modal.btn.copy.all') }}</span>
          </div>
          <div class="link-m-b-btn-i btn-bg" @click="handelCopyLink">
            <span>{{ t('mst.invite.link.modal.btn.copy.link') }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  const handleGetLinkInfo = async () => {
    const res: any = await ThMeetingStore.ThImEvent.getMeetingShareInfo({
  ...
  const handleCopyAll = () => {
    const time =
      linkInfo?.value.planType === 1 || linkInfo?.value.planType === 0
        ...
        : t('mst.invite.link.modal.text6');
    const message = `${linkInfo.value.name} ${t(
      'mst.invite.link.modal.text1'
    )} \n${t('mst.invite.link.modal.text2')}: ${linkInfo.value.subject} \n${t(
      'mst.invite.link.modal.text5'
    )}: : ${time}  \n${t('mst.invite.link.modal.text4')}: ${
      linkInfo.value.meetingNo
    }\n${t('mst.invite.link.modal.text3')}: ${linkInfo.value.inviteUrl} `;
    if (isWujie.value) {
      Copy(message);
      Msg.success(t('mst.invite.link.modal.msg.copy.success'));
      return;
    }
    navigator.clipboard
      .writeText(message)
      ...
      .catch((err: any) => {
        console.log(err);
        Msg.error(t('mst.invite.link.modal.msg.copy.fail'));
      });
  };
